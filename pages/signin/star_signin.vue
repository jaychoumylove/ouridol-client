	...
	<view class="signin-container">
		...
		<view class="signin-header">
			<view class="signin-avurl" style="background:url(https://mmbiz.qpic.cn/mmbiz_png/w5pLFvdua9EN7TlaOkWO9nGX11LnKJ8GPxdSoYiaIg8Q8rGpTfiapibE78n5A8oXLJn2pxMkIR41PEgwmBdibrOib6A/0) no-repeat ;background-size: 100% 100%;">
				...
				<view class="signin-button">
					...
					<view :class="index==1 ? 'buttons':'button'" :style="{background: 'url('+item.image+') no-repeat; background-size: 100% 100%;'}">
						...
						<button class="signin-button-center" ><image src="/static/image/signin/Invalid_play.png"></button>
						</view> 
						<view v-else-if="(item.status==2)&&(sign)" class="play_img" >
							<button class="signin-button-center"><image src="/static/image/signin/already_play.png"></button>
						</view>
						
						<view v-else-if="(item.status==2)&&(!sign)" class="play_img">
						<button class="signin-button-center"  @tap.stop="modelSignin()"><image src="/static/image/signin/play.png"></button>
						</view>
						<view v-else class="play_img">
						<button class="signin-button-center"><image src="/static/image/signin/no_play.png"></image></button>
						</view>
					</view>
					</block>
				</view>
			...
			<modalComponent v-if="modal == 'signins'" @closeModal="modal=''">
				<view style="height: 100%;width: 100%;margin: 0 auto;">
					...
					<view style="text-align:center;"><image src="../../static/image/user/b1.png" style="width: 50upx;height: 50upx;"></image>能量+{{coin}}</view>
					<button @tap="$app.goPage('/pages/group/group')" style="text-align: center;margin-top: 20upx;"><image style="width: 320upx; " src="/static/image/signin/sendcoin.png"  mode="widthFix"></image></button>
					...
					<view style="height: 100%;width: 100%;text-align: center;padding: 20rpx;">
						...
						<view class="flex-center" style="margin-top: 20upx;">
							<view class="play_border">
								...
								<button @tap="joinSignin(1)"><image style="width: 165upx;margin-top: 20upx;" src="/static/image/signin/video_play.png" mode="widthFix"></image></button>
							</view>
							<view class="play_border">
								<image src="/static/image/user/b1.png" style="width: 140upx;height: 150upx;"></image>
								<view>+100能量</view>
								<button @tap="joinSignin(0)"><image style="width: 165upx;margin-top: 20upx;" src="/static/image/signin/plays.png" mode="widthFix"></image></button>
							</view>
						</view>
					</view>
		...
		<view class="signin-body center" >
			<view class="sign" style="background: url(https://mmbiz.qpic.cn/mmbiz_png/w5pLFvdua9EN7TlaOkWO9nGX11LnKJ8GtLo0nzIibQ9JbIhNqerQCkVEtTUGMwVSm6FNf68SDSetQWicPPAKf3Nw/0)no-repeat ;background-size: 100% 100%;">
				<button v-if="!sign&&!complete" @tap.stop="modelSignin()"><view class="play_sign">签到</view></button>
...
<script>
	...
	import btnComponent from '@/components/btnComponent.vue'
		export default {
			...
			methods: {
				getSignin() {
					this.$app.request('rank/getSignin', {starid:this.starid}, res => {
...
<style lang="scss" scoped>
	...
	.signin-button-center{margin: 0 auto;display:table-cell; vertical-align:bottom;
		image{width: 125upx;height: 45upx;}
	}
	.signin-container {height: 100%;padding:25upx;color: #000000;}
	.star-avurl{width:200upx;margin: 0 auto;
		image{width:180upx;height:180upx;border-radius: 50%; border: 3upx solid #D8B82D;margin-top:20upx}
	}
	.star-name{font-size: 30upx;color: #FFFFFF;font-weight: 500; letter-spacing:10upx;margin: 25upx 0;height: 45rpx;display: block;}
	.center{text-align: center;}
	.rule{color: #FFFFFF;margin-right: 20upx;}
	.rule::after {color: #FFFFFF;content: "\e64c";font-family: "iconfont" !important;padding-left: 10upx;}
	.play_img{height:75%;display:flex;align-items:flex-end}
	.signin-avurl{height: 400upx;border-radius: 20upx;}
	.signin-button{display: flex;-webkit-box-pack: justify;-webkit-justify-content: space-between;justify-content: space-between;}
	.signin-time{color: #666666;width: 190rpx;margin: 0 auto;}
	.time{width: 50upx;height: 40upx;background-color: #000000;text-align: center;line-height: 40upx;margin: 0 1upx;display: block; color:#FFFFFF;border-radius: 8upx;letter-spacing:5upx;}
	.font-size-lg{font-size: 30upx;}
	.font-size-bg{font-size: 35upx;}
	.font-size-xs{font-size: 20upx;}
	.margin-ls{margin: 20upx auto;}
	.flex{display: flex;-webkit-box-pack: justify;-webkit-justify-content: space-between;justify-content: space-between;}
	.signin-body{margin-top: 20upx;}
	.sign{height: 255upx;width: 250rpx;margin: 0 auto;margin-top: 250rpx;text-align: center;}
	.play_sign{height: 100%;line-height: 250rpx;font-size: 45rpx;color: #FFFFFF;letter-spacing:5upx;}
	.play_border{width: 48%;height: 300upx; border:4upx solid #2B93FE; text-align: center;margin: 10upx;border-radius: 20upx;}
	.rocket{font-size: 40upx;font-weight: bold;margin: 10rpx auto;}
	
	.button{width: 250upx;height:280upx; padding: 20upx; color: #FFFFFF;margin-top: 35upx;}
	.buttons{width: 250upx;height:280upx; padding: 20upx; margin: 0 10upx;color: #FFFFFF;}
	.gift{margin: 20upx;}
	.gift-info{width: 200upx; height: 280upx; border-radius: 10upx;padding: 10upx;
	image{height: 65%;}
	}
	.gitf-font{font-size: 28upx;color: #E31076; text-align: center;border-bottom: 2upx solid #FACED2;margin: 0 auto;width: 140upx;font-weight: 500;}
	.gift-but{text-align:center;font-size: 20rpx;background-color: #FE737A; width: 150upx;height: 40upx;color: #FFFFFF;border-radius: 10upx;line-height: 40upx;margin: 10upx auto;}
	.signin-footer{margin-top: 20upx;}
	.footer-flex{display: flex;flex-flow: row nowrap;justify-content: space-between;}
	.shara-but{font-size: 24upx;width: 150upx;height: 70upx;background-color: #007AFF;border-radius: 20upx;line-height: 70upx;color: #FFFFFF;}
	.ava-flex{display: flex;flex-wrap: wrap;}
	.avatarurl{
		width: 140upx;text-align: center;
		image{width: 100upx;height: 100upx;border-radius: 50%;margin:20upx;}
	}
	.user_image{height:350upx; margin-top: 20upx; background: linear-gradient(#D5A7EB, #EAB2DD);border-radius: 20upx;} 
	.signin{margin-top: 65upx;}
